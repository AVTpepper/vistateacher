...
| ---------------------- | -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `users/{uid}`          | Server-created public educator profile | normalized identity/location, professional roles, education stage, subjects, taught languages, verification, counters, status, timestamps |
...
Resource reservations store `status: uploading`, the generated `filePath`, expected MIME and size, `usagePeriod`, and pending moderation state while incrementing `usage/{uid}_{YYYY-MM}.resourceUploads`. Community downloads increment `usage/{uid}_{YYYY-MM}.resourceDownloads` transactionally after the private object is readable; owners are exempt and Plus is unlimited. Finalization verifies the private object and changes status to `active` with approved moderation. Reviews use `{resourceId}_{uid}` IDs and transactionally maintain `ratingTotal`, `ratingCount`, and `ratingAverage`. Resource objects remain private in Storage and are served only by the entitlement-checked attachment route.
...
AI generation reserves `usage/{uid}_{YYYY-MM}.aiLessons` plus the matching `aiLessonCreations` or `aiRefinements` counter and lesson generation state in one transaction. Community accounts receive one creation and two refinements within a three-generation monthly ceiling; Plus retains fifty total generations. The usage document also stores the trusted last-generation timestamp used for cross-instance rate enforcement. A successful model response writes validated current content and `versions/v{number}` atomically. Failed generation restores the previous ready lesson and decrements both counters. Community lesson exports reserve `lessonExports` transactionally and release failed renders; manual edits and duplication do not consume AI quota.
