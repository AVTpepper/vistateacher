...
import { startConversationSchema } from "@/schemas/messages";
...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({
    conversations: await getConversationSummaries(account.uid),
  });
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    return NextResponse.json(
      await startConversation(account.uid, parsed.data),
      { status: 201 },
    );
  } catch (error) {
